...
def test(cfg,
    ...
    if model is None:
        ...
        # Remove previous
        for f in glob.glob('./genshin/test_batch*.png'):
            os.remove(f)
    ...
    for batch_i, (imgs, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        f = './genshin/test_batch%g.png' % batch_i  # filename
    ...
    if save_json and map and len(jdict):
        ...
        with open('./genshin/results.json', 'w') as file:
        ...
        cocoDt = cocoGt.loadRes('./genshin/results.json')  # initialize COCO pred api
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./genshin/weights/best.pt', help='weights path')
    ...
    if opt.task == 'test':  # (default) test normally
        ...
        np.savetxt('./genshin/benchmark.txt', y, fmt='%10.4g')  # y = np.loadtxt('study.txt')
        ...
        for i in x:
            ...
            y.append(r + (time.time() - t,))
        np.savetxt('./genshin/study.txt', y, fmt='%10.4g')  # y = np.loadtxt('study.txt')
        ...
        fig.tight_layout()
        plt.savefig('./genshin/study.jpg', dpi=200)
