...
wdir = './genshin/weights' + os.sep  # weights dir
last = wdir + 'last.pt'
best = wdir + 'best.pt'
...
def train():
    ...
    temp_look = dict(model.named_parameters())
    ...
    if opt.adam:  #2种优化策略可供选择   公认Adam比较好
    ...
    # 学习率调整策略  注意 这个学习率调整是在每一轮（epoch）结束之后进行调整的
    ...
    dataloader = torch.utils.data.DataLoader(dataset,
                                             ...
                                             num_workers=nw,
                                             shuffle=not opt.rect,  #not opt.rect  Shuffle=True unless rectangular training is used
                                             pin_memory=True,
    ...
    model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device)  # attach class weights 附加类权重
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            # targets shape[39,6] batch class x y w h
            ...
            # Run model
            pred = model(imgs)  #[8,3,26,26,8] [8,3,13,13,8]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./genshin/cfg/yolov3-tiny-3cls.cfg', help='*.cfg path')
    parser.add_argument('--data', type=str, default='./genshin/data/dnf.data', help='*.data path')
    ...
    parser.add_argument('--weights', type=str, default='./genshin/weights/yolov3-tiny.conv.15', help='initial weights path')
